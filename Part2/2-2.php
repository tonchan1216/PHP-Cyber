<?php
	...
	if ( ! empty( $_POST["dopost"] ) && ! empty( $_POST["name"] ) ) {
		...
		// 名前とパスワードの一致確認
		$sql = "SELECT COUNT(*) FROM bbs WHERE name='".addslashes($_POST["name"])."' AND pass<>'".addslashes($_POST["pass"])."'" ;
		...
		// 投稿内容のデータベースへの書込み
		$sql = "INSERT INTO bbs SET name='".addslashes($_POST["name"])."', title='".addslashes($_POST["title"])."', body='".addslashes($_POST["body"])."', pass='".addslashes($_POST["pass"])."', postdate=NOW()" ;
	...
	if ( ! empty( $_POST['delete'] ) ) {
		...
		$sql = "DELETE FROM bbs WHERE id=".intval($_POST["id"])." AND pass='".addslashes($_POST["pass"])."' LIMIT 1" ;
		mysql_query( $sql , $conn ) ;
	...
	if ( ! empty( $_GET['message'] ) ) {
		echo '<p style="color:red;">'.htmlspecialchars($_GET['message'],ENT_QUOTES).'</p>' ;
	}
